...
async function enrichWithCourseDetails(items, courseIdGetter) {
  ...
  const ids = Array.from(
    new Set(
      list.map(courseIdGetter).filter((v) => v !== null && v !== undefined),
    ),
  );
  ...
  selectParts.push(
    titleCol ? `${titleCol} AS course_title` : `NULL AS course_title`,
  );
...
export async function postStudentReset(req, res) {
  try {
    ...
    await pool.query(`UPDATE public_users SET password_hash = ? WHERE id = ?`, [
      pw,
      rows[0].id,
    ]);
...
export async function studentDashboard(req, res) {
  ...
  if (studentId) {
    ...
    recentAttendance = await enrichWithCourseDetails(
      att || [],
      (a) => a.course_id,
    );
  }
  ...
  try {
    if (studentId) {
      ...
      for (const row of resolved.rows || []) {
        ...
        const category = categorizePayment(row.name, row.purpose);
